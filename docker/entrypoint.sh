...
/app/snowflake --public-ip 127.0.0.1 --http-port $HTTP_PORT --rpc-port $RPC_PORT --max-peers 0 \
  --bootstrappers-path "$BOOTSTRAPPERS_FILE" --light-bootstrappers-path "$PEER_FILE" ${PEER_ARGS:+"$PEER_ARGS"} \
  --network-id local
